...
function goToVeterinary() {
  return navigateTo(localePath('/services/veterinary'))
}
</script>
...
<template>
  <BaseErrorState
    ...
    variant="brand"
    :title="t('site.profile.veterinary.errorTitle')"
    :message="t('site.profile.veterinary.errorDescription')"
    @retry="refetch"
  />
  ...
  <div v-else class="space-y-6 sm:space-y-7">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
      <BaseStatCard
        ...
        :label="t('site.profile.veterinary.summaryLabel')"
        :value="totalCount"
    ...
    <div class="relative space-y-3">
      ...
      <BaseEmptyState
        ...
        variant="brand"
        icon="lucide:stethoscope"
        :title="t('site.profile.veterinary.emptyTitle')"
        :description="t('site.profile.veterinary.emptyDescription')"
      >
        <div class="flex flex-wrap items-center justify-center gap-2">
          <BaseButton variant="outline" size="sm" class="!rounded-xl" @click="resetFilters">
            {{ t('site.profile.veterinary.clearFilters') }}
          </BaseButton>
          <BaseButton variant="brand" size="sm" class="!rounded-xl !px-4 !py-2.5" @click="goToVeterinary">
            <Icon name="lucide:stethoscope" class="size-4" aria-hidden="true" />
            {{ t('site.profile.veterinary.browseClinics') }}
          </BaseButton>
        </div>
      </BaseEmptyState>

      <template v-else>
        <ProfileVeterinaryCard
          v-for="item in items"
          :key="item.id"
          :item="item"
